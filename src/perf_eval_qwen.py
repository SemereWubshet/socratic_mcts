...
VISUAL_SETTINGS = {
        "qwen3:8b": {
        "color": Category10[10][6],
        "display_name": "Qwen3"
    },
    "Qwen-STF": {
        "color": Category10[10][7],
        "display_name": "Qwen3 (STF)"
    },
}
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame()
    for name in glob.glob(str(input_dir / "eval_*wen*.json")):
        with open(name) as f:
    ...
    export_svgs(p1, filename="./figs/qwen_success_rate_by_interaction_length.svg")
    ...
    export_svgs(p2, filename="./figs/qwen_success_rate_by_persona.svg")
    ...
    export_svgs(p3, filename="./figs/qwen_success_rate_by_opening_question_type.svg")
    ...
    export_svgs(p4, filename="./figs/qwen_success_rate_by_model_size.svg")
