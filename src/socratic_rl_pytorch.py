...
def rollout(
    ...
    torch.cuda.memory._dump_snapshot(f"{output_path}/memory_profile_rollout.pkl")
...
def vf_rollout(dataset_path: pathlib.Path, action_vf_path: str, output_path: pathlib.Path, device: str) -> None:
    ...
    torch.cuda.memory._dump_snapshot(f"{output_path}/memory_profile_vf_rollout.pkl")
    torch.cuda.memory._record_memory_history(enabled=None)
...
def vf_train(
    ...
    torch.cuda.memory._dump_snapshot(f"{vf_output_path}/memory_profile_vf_train.pkl")
    torch.cuda.memory._record_memory_history(enabled=None)
...
def policy_train(
    ...
    torch.cuda.memory._dump_snapshot(f"{output_dir}/memory_profile_policy_train.pkl")
    torch.cuda.memory._record_memory_history(enabled=None)
