...
def rollout(
    ...
    nemo = OllamaAgent(model="mistral-nemo", client=Client(ollama_client))
...
def vf_rollout(dataset_path: pathlib.Path, action_vf_path: str, output_path: pathlib.Path, device: str) -> None:
    ...
    for item in evaluations_dataset.root:
        ...
        # Ends with a teacher interaction
...
def policy_train(
    ...
    for item in evaluations_dataset.root:
        history = item.interaction.chat_history.root
        # Ends with a student interactions
    ...
    for i in range(args.num_iterations):
        ...
        if not interactions_path.exists():
            p = Process(
                target=rollout,
                args=(
                    seeds_path, str(current_policy_path), interactions_path, "cuda", args.ollama_client,
        ...
        if not evaluations_path.exists():
            ...
            print("", flush=True)
            print(f"Avg. perf. {i}: {evaluations_dataset.avg_performance()}", flush=True)
            print("", flush=True)

        if not action_vfn_model_dir.exists():
            for j in range(args.vf_training_it):
                vf_training_path = train_it_dir / "vf_training"
                current_vf_path = current_vf_path if j == 0 else vf_training_path / f"it_{j - 1}" / "value_fn"

                vf_target_path = (
                    vf_training_path / f"it_{j}" / "value_fn" if j < args.vf_training_it - 1 else action_vfn_model_dir
                )

                dataset_path = vf_training_path / f"it_{j}" / "dataset"
                p = Process(target=vf_rollout, args=(evaluations_path, str(current_vf_path), dataset_path, "cuda"))
                p.start()
                p.join()
                assert p.exitcode == 0
                p.close()
                ...
                p = Process(
                    target=vf_train,
                    args=(dataset_path, value_checkpoints / f"it_{j}", str(current_vf_path), vf_target_path)
                )
                p.start()
                p.join()
                assert p.exitcode == 0
        ...
        #     )
        # )
        print("starting policy train")
        policy_train(evaluations_path,
                     policy_checkpoints,
                     str(action_vfn_model_dir),
                     str(current_policy_path),
                     policy_model_dir)
